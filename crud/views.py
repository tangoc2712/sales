...
def orders(request):
    if request.user.is_anonymous:
        return HttpResponse(
            json.dumps({"detail": "Not authorized"}),
    ...
    if request.method == "GET":
        ...
        orders_data = [OrderSerializers(order).data for order in orders_data]
        return HttpResponse(
...
def order(request, order_id):
    ...
    if request.method == "GET":
        return HttpResponse(
            json.dumps({"data": OrderSerializers(order).data}),
            status=status.HTTP_200_OK,
        )
...
def save_order(request, order, success_status):
    ...
    return HttpResponse(
        json.dumps({"data": OrderSerializers(order).data}), status=success_status
    )
